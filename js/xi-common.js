...
function moveFocus (element, dir, vdir) {
    ...
    if (htmlobj.tagName.toLowerCase() == 'td' && vdir){
       ...
       for (var newRow=htmlrow.rowIndex - (htmltable.childNodes[0]).rowIndex + dir; newRow >= 0 && newRow < htmltable.rows.length; newRow+=dir){
         var targetCell=htmltable.rows[newRow].cells[htmlobj.cellIndex];
         targetCell=targetCell.childNodes[0];
         if (targetCell && getStyle(targetCell,'display')!='none')
         for (var i=0; i<targetCell.childNodes.length; i++) {
    ...
    for (var next = getIndex(element) + dir; next<element.form.length && next>=0; next+=dir) {
        var target = element.form.elements[next];
        var targetTag= new String (target.tagName);
        targetTag=targetTag.toLowerCase()
